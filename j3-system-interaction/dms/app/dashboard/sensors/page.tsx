...
const SENSOR_NODES: SensorNode[] = [
  { id: 'SNS-KEL-001', location: 'Kelani Basin - Hanwella', district: 'Colombo', metric: 'Water Level', status: 'ONLINE', battery: 87, lastSeenMinutes: 1, latestValue: '4.6 m' },
  { id: 'SNS-KAL-014', location: 'Kalu Ganga - Ratnapura Bridge', district: 'Ratnapura', metric: 'Water Level', status: 'ONLINE', battery: 73, lastSeenMinutes: 2, latestValue: '5.4 m' },
  { id: 'SNS-ATT-005', location: 'Attanagalu Oya - Gampaha', district: 'Gampaha', metric: 'River Flow', status: 'ONLINE', battery: 65, lastSeenMinutes: 3, latestValue: '208 m3/s' },
  { id: 'SNS-GIN-021', location: 'Gin Ganga - Galle North', district: 'Galle', metric: 'Rainfall', status: 'MAINTENANCE', battery: 41, lastSeenMinutes: 18, latestValue: '72 mm / 24h' },
  { id: 'SNS-MAT-008', location: 'Nilwala Basin - Matara', district: 'Matara', metric: 'Rainfall', status: 'OFFLINE', battery: 0, lastSeenMinutes: 120, latestValue: 'No signal' },
  { id: 'SNS-KEG-002', location: 'Mawanella Slope Station', district: 'Kegalle', metric: 'Soil Moisture', status: 'ONLINE', battery: 92, lastSeenMinutes: 1, latestValue: '38%' },
];
...
export default function SensorsPage() {
  ...
  const totals = useMemo(() => {
    ...
    const averageBattery = Math.round(
      sensors.reduce((sum, sensor) => sum + sensor.battery, 0) / (sensors.length || 1),
    );
  ...
  const saveSensor = () => {
    ...
    const latestValue = draft.latestValue.trim();

    if (!id || !location || !district || !latestValue) {
      setNotice({ type: 'error', text: 'Please complete all required fields.' });
      return;
    }

    const duplicate = sensors.some(
      (sensor) => sensor.id.toLowerCase() === id.toLowerCase() && sensor.id !== editorOriginalId,
    );
    if (duplicate) {
    ...
    if (editorMode === 'add') {
      setSensors((prev) => [{ ...draft, id, location, district, latestValue }, ...prev]);
      setNotice({ type: 'success', text: `Sensor ${id} added.` });
    } else if (editorMode === 'edit' && editorOriginalId) {
      setSensors((prev) =>
        prev.map((sensor) =>
          sensor.id === editorOriginalId ? { ...draft, id, location, district, latestValue } : sensor,
        ),
      );
    ...
    <div className="flex-1 overflow-y-auto bg-[#0a0f16] text-white">
      <div className="p-8 max-w-[1400px] mx-auto">
        <div className="flex items-end justify-between mb-8">
          <div>
            ...
            <p className="text-sm text-slate-400">
              Real-time telemetry from river basins and high-risk catchment zones.
            </p>
          </div>
          <button
            onClick={openAddEditor}
            className="px-4 py-2.5 rounded-lg bg-blue-500/20 hover:bg-blue-500/30 border border-blue-500/40 text-sm font-semibold text-blue-300 flex items-center gap-2 transition-colors"
          >
            <Plus size={14} />
            Add Sensor
          </button>
          ...
          <div className="bg-[#131924] border border-slate-800/80 rounded-xl p-5 mb-6">
            <div className="flex items-center justify-between mb-4">
              <h2 className="text-sm font-bold tracking-widest uppercase text-slate-300">
                {editorMode === 'add' ? 'Add Sensor' : 'Edit Sensor'}
              </h2>
              <button onClick={closeEditor} className="text-slate-500 hover:text-slate-300 transition-colors">
                <X size={16} />
              </button>
            </div>
            <div className="grid grid-cols-3 gap-4">
              <Field label="Sensor ID">
                <input
                  value={draft.id}
                  onChange={(event) => setDraft((prev) => ({ ...prev, id: event.target.value.toUpperCase() }))}
                  className="w-full bg-[#0a0f16] border border-slate-800 rounded-lg px-3 py-2 text-sm text-slate-200 focus:outline-none focus:border-blue-500/50"
                  placeholder="SNS-KEL-099"
                />
              </Field>
              <Field label="Metric">
                <select
                  value={draft.metric}
                  onChange={(event) => setDraft((prev) => ({ ...prev, metric: event.target.value as SensorMetric }))}
                  className="w-full bg-[#0a0f16] border border-slate-800 rounded-lg px-3 py-2 text-sm text-slate-200 focus:outline-none"
                >
              ...
              <Field label="Status">
                <select
                  value={draft.status}
                  onChange={(event) => setDraft((prev) => ({ ...prev, status: event.target.value as SensorStatus }))}
                  className="w-full bg-[#0a0f16] border border-slate-800 rounded-lg px-3 py-2 text-sm text-slate-200 focus:outline-none"
                >
              ...
              <Field label="Location">
                <input
                  ...
                  className="w-full bg-[#0a0f16] border border-slate-800 rounded-lg px-3 py-2 text-sm text-slate-200 focus:outline-none focus:border-blue-500/50"
                  placeholder="Kelani Basin - Hanwella"
                />
              </Field>
              ...
              <Field label="Latest Reading">
                <input
                  value={draft.latestValue}
                  onChange={(event) => setDraft((prev) => ({ ...prev, latestValue: event.target.value }))}
                  className="w-full bg-[#0a0f16] border border-slate-800 rounded-lg px-3 py-2 text-sm text-slate-200 focus:outline-none focus:border-blue-500/50"
                  placeholder="4.2 m"
                />
              </Field>
              <Field label="Battery %">
                <input
                  ...
                  value={draft.battery}
                  onChange={(event) =>
                    setDraft((prev) => ({ ...prev, battery: Math.max(0, Math.min(100, Number(event.target.value))) }))
                  }
                  className="w-full bg-[#0a0f16] border border-slate-800 rounded-lg px-3 py-2 text-sm text-slate-200 focus:outline-none focus:border-blue-500/50"
                />
              </Field>
              <Field label="Last Seen (min)">
                <input
                  type="number"
                  min={0}
                  value={draft.lastSeenMinutes}
                  onChange={(event) =>
                    setDraft((prev) => ({ ...prev, lastSeenMinutes: Math.max(0, Number(event.target.value)) }))
                  }
                  className="w-full bg-[#0a0f16] border border-slate-800 rounded-lg px-3 py-2 text-sm text-slate-200 focus:outline-none focus:border-blue-500/50"
                />
              </Field>
            ...
            <div className="flex items-center justify-end gap-3 mt-5">
              <button
                onClick={closeEditor}
                className="px-4 py-2 rounded-lg border border-slate-700 text-slate-300 text-sm font-semibold hover:bg-slate-800/60 transition-colors"
              >
                Cancel
              </button>
              <button
                onClick={saveSensor}
                className="px-4 py-2 rounded-lg border border-blue-500/40 bg-blue-500/20 text-blue-300 text-sm font-semibold hover:bg-blue-500/30 transition-colors"
              >
                {editorMode === 'add' ? 'Create Sensor' : 'Save Changes'}
              </button>
            </div>
          </div>
        )}
        ...
        <div className="grid grid-cols-4 gap-6 mb-6">
          <StatCard title="Total Nodes" value={totals.total.toString()} color="blue" icon={<RadioTower size={14} />} />
        ...
        <div className="bg-[#131924] border border-slate-800/80 rounded-xl p-4 flex gap-4 mb-6">
          ...
          <select
            value={statusFilter}
            onChange={(event) => setStatusFilter(event.target.value as 'ALL' | SensorStatus)}
              ...
              {filteredSensors.map((sensor) => (
                <tr key={sensor.id} className="hover:bg-slate-800/20 transition-colors">
                  <td className="px-6 py-4">
                    ...
                    <p className="text-[10px] text-slate-500 mt-1 font-semibold uppercase tracking-wide">{sensor.metric}</p>
                  </td>
                  <td className="px-6 py-4">
                    <p className="text-xs font-semibold text-slate-300">{sensor.location}</p>
                    <p className="text-[10px] text-slate-500 flex items-center gap-1 mt-1">
                      <MapPin size={10} /> {sensor.district}
                    </p>
                  </td>
                  <td className="px-6 py-4">
                  ...
                  <td className="px-6 py-4">
                    <span className={`px-2.5 py-1 rounded text-[9px] font-bold tracking-widest border ${STATUS_STYLES[sensor.status]}`}>
                      {sensor.status}
                    </span>
                  </td>
                  <td className="px-6 py-4 text-center">
                    <div className="inline-flex items-center justify-center gap-2">
                      <div className="w-16 h-1.5 bg-slate-800/80 rounded-full overflow-hidden">
                        <div
                          className={`${sensor.battery >= 60 ? 'bg-green-400' : sensor.battery >= 30 ? 'bg-orange-400' : 'bg-red-400'} h-full rounded-full`}
                  ...
                  <td className="px-6 py-4 text-center">
                    <div className="inline-flex items-center justify-center gap-2">
                      <button
                        onClick={() => openEditEditor(sensor)}
                        className="px-2.5 py-1.5 rounded-md border border-slate-700 text-slate-300 hover:bg-slate-800/70 transition-colors text-xs font-semibold flex items-center gap-1"
                      >
                        <Pencil size={12} />
                        Edit
                      </button>
                      <button
                        ...
                        className="px-2.5 py-1.5 rounded-md border border-red-500/30 text-red-300 hover:bg-red-500/10 transition-colors text-xs font-semibold flex items-center gap-1"
                      >
                        <Trash2 size={12} />
                        Remove
                      </button>
              ...
              {filteredSensors.length === 0 && (
                <tr>
                  <td colSpan={6} className="px-6 py-12 text-center text-slate-500 text-sm">
                    No sensors found matching the current filters.
                  </td>
                </tr>
              )}
            </tbody>
